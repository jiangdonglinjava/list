...
public class bmob_file extends BaseActivity implements OnClickListener, FileChooserListener
{
	...
	{
		if(v.getId()==R.id.tv_one_one){//插入单条数据（一个BmobFile列）
				insertDataWithOne();}
				else if(v.getId()==R.id.tv_one_many){//插入单条数据（多个BmobFile列）
				insertDataWithMany();}
				else if(v.getId()==R.id.tv_many_one){//批量插入多条数据-且每条数据都存在一个BmobFile列
				insertBatchDatasWithOne();}
				else if(v.getId()== R.id.tv_many_many){//批量插入多条数据-且每条数据都存在多个BmobFile列
				insertBatchDatasWithMany();}
				else if(v.getId()== R.id.btn_delete){//
				deleteFile();}
				else if(v.getId()== R.id.btn_delete_batch){//批量删除
				deleteBatchFile();}
	}
